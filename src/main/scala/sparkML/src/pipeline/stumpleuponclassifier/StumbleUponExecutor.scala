package sparkML.src.pipeline.stumpleuponclassifier
...
object StumbleUponExecutor {
  ...
  def main(args: Array[String]) {
    ...
    // val command = args(0)
    // LR - LogisticRegression, DT - DecisionTree, RF - RandomForest, GBT - GradientBoostedTree, NB - NaivesBayes, SVM - SVM
    val command = "RF"
    ...
    val dataFilePath = baseDir + "/src/main/resources/sparkML/pipeline/train.tsv"

    // get dataframe
    val df = sqlContext.read.format("com.databricks.spark.csv").option("delimiter", "\t").option("header", "true")
      .option("inferSchema", "true").load(dataFilePath)
