...
object ex_LinearRegression {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession
      .builder()
      .appName("Spark Linear Regression").config(spConfig)
      .getOrCreate()
  ...
  def executeLR(spark: SparkSession) {
    ...
    val dataFilePath2 = baseDir + "/src/main/scala/sparkML/data/ProductSalesTest.csv"
    var testDf = spark.read.format("csv").option("header", "true").load(dataFilePath2)

    println("First 5 rows of test data")

    testDf.show(5)

    // Defining UDF to change category from string to integer
    ...
    val removeFirstCharOrLastCharUDF = udf((str:String) => {
      if (str != null)
        if(str.charAt(0)=='P')
          str.slice(1,str.length).toInt
        else if(str.charAt(str.length()-1)=='+')
          str.slice(0,str.length()-1).toInt
        else str.toInt
      else
        0
    })
    ...
    //Creating RFormula of features and label columns
...
//    val formula = new RFormula().setFormula("""Purchase ~ Product_ID+Gender+Occupation+City_Category+Stay_In_Current_City_Years+Marital_Status""").setFeaturesCol("features").setLabelCol("label")
    ...
    // if using this formula, the drop the columns: setFormula("""Purchase ~ Product_ID+Gender+Occupation+City_Category+Stay_In_Current_City_Years+Marital_Status""").setFeaturesCol("features").setLabelCol("label")
//    trainDf = trainDf.drop("Product_Category_1", "Product_Category_2", "Product_Category_3")
//    testDf = testDf.drop("Product_Category_1", "Product_Category_2", "Product_Category_3")
    ...
    train.select(train.col("features"), train.col("label")).show(10, false)
